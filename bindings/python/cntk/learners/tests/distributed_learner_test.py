...
class SimpleTrainer:
    def __init__(self, mode, config, numLearners=1):
        print("In SimpleTrainer Init *****************")
    ...
    def create_model(self):
        ...
        self.p1 = C.parameter(shape=(self.input_dim, self.embed_dim), init=1)
        self.p2 = C.parameter(shape=(self.embed_dim, self.output_dim), init=1)    
        o = C.times(i, self.p1)
        q = C.times(o, self.p2)
        self.z = C.reduce_sum(q)

    def create_trainer(self, mode, config):
        distributed_learner = self.create_distributed_learner(mode, config)
        self.trainer = C.Trainer(self.z, (self.z, None), distributed_learner, []) if distributed_learner else None

    def create_distributed_learner(self, mode, config):
        
        if(self.numLearners > 1):
            assert(len(self.z.parameters) >= self.numLearners)
    
        local_learners = []        
        for i in range(self.numLearners):
            local_learners.append(C.sgd([self.z.parameters[i]], C.learning_parameter_schedule_per_sample(0.01)))
      
        try:
            if mode == 'data_parallel':
                if config is None:
                    config = DataParallelConfig(num_quantization_bits=32, distributed_after=0)
                learner = C.data_parallel_distributed_learner(local_learners[0], num_quantization_bits=config.num_quantization_bits, distributed_after=config.distributed_after)
                ...
                learner = C.block_momentum_distributed_learner(local_learners[0], block_momentum_as_time_constant=config.block_momentum_as_time_constant, block_learning_rate=config.block_learning_rate, block_size=config.block_size, distributed_after=config.distributed_after)
            else:
                learner = local_learners[0]
        except RuntimeError:
            learner = None
        return learner
...
def distributed_worker(outdir, gpu, mode, config):
    ...
    np.save(os.path.join(outdir, mode+str(C.Communicator.rank())), trainer.p2.value)
...
def test_distributed_training_accuracy(tmpdir, device_id, mode, config):
    ...
    str_out =mpiexec_execute(__file__, ['-n', str(NUM_WORKERS)], launch_args)
    print(str_out)
    ...
    assert np.allclose(p0, ref_trainer.p2.value)
